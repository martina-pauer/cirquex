class Suscription():
    ...
    def duplicate(self, mult : int) -> float:
        '''
...
class FinancialCirq():
    ...
    def cirq_view(self, file_name : str, author : str, css_text : str):
        ...
        copy = copy.replace('#3', css_text.__str__() + 'header, footer {padding: -98%; text-align: center;} .rect, .circle {transition: 1.8s ease-in; display: inline-grid; border: 0.3em solid #000000; margin: 2%; padding: 1em;} .circle {border-radius: 1.4em;} .circle:hover, .rect:hover {background: #dddddd; padding-top: 4%; padding: -2%; font-size: 1.2em; text-align: center; height: 4em; width: 7em;} * .circle:hover, * rect:hover {background: #000000; color: #ffffff;}')
